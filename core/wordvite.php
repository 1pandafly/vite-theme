...
/*
|--------------------------------------------------------------------------
| Wordvite v0.1.0 Helper Functions
|--------------------------------------------------------------------------
*/
...
/**
 *
 * Add module or nomodule type to vite dist
 *
 */
...
{
    $file_path = get_template_directory() . '/dist/img/' . $name;
    $file_type = mime_content_type($file_path);
    $file_name = pathinfo($name, PATHINFO_FILENAME);;
    $file_path_webp = get_template_directory() . '/dist/img/' . $file_name . '.webp';
    $file_size = getimagesize($file_path);

    if (file_exists($file_path)) {
        $original_url = get_template_directory_uri() . '/dist/img/' . $name;
    }
    if (file_exists($file_path_webp)) {
        $webp_url = get_template_directory_uri() . '/dist/img/' . $file_name . '.webp';
    }
...
{
    $css_file = get_template_directory() . "/dist/css/" . $style_name . ".css";
...
function wv_load_script($chunk_name)
{
    ...
    if ($is_watch) {
        ...
        // css if have
        $css = isset($manifest["js/" . $chunk_name . ".js"]["css"]) ? $manifest["js/" . $chunk_name . ".js"]["css"] : null;
        if ($css) {
